# === From full_memecoin_tool_dashboard_hint.py ===
...
def scan_loop():
    ...
    while True:
        ...
        msg = f"Keine neuen Memecoins gefunden – Stand {now}"
        try:
            bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=msg)
        except Exception as e:
            print("Telegram-Fehler bei leerem Ergebnis:", e)
    time.sleep(600)
...
# === From full_memecoin_tool_without_twitter.py ===
...
# === Hole neue Tokens von CoinGecko mit Rankings ===
def get_new_tokens(limit=5):
    url = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_asc&per_page={}&page=1&sparkline=false".format(limit)
    try:
        res = requests.get(url)
...
# === Hintergrundprozess: Alle 10 Minuten scannen ===
def scan_loop():
    global latest_scan
    print("Starte Hintergrund-Scan...")
    while True:
